package com.inigrey.android.opengles.gltext;
...
public class GLText {
    ...
    // R: [none]
    public float draw(String text, float x, float y) {
        float startX = x;
        float startY = y;
    ...
    public float width(String text) {
        ...
        float x = (chrWidth / 2.0f) - (fontPadX * scaleX);
        int len = text.length();
        for (int i = 0; i < len; i++) {
